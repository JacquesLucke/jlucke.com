...
description: Sharing some insights into how I built my personal website.
pubDate: 2025-06-12
status: draft
---

After years of having a website that just said "I don't have a website yet" I finally got my act together and built a proper personal website. In this first blog post I want to share my motivation for building it as well as its current technical details.

## Motivation

Over the last few years, I've often thought about writing about the projects I do as part of work and on my own time. My main motivation for that is to share what I've worked on and thought about with my future self and other curious people. I often talk to individuals about these projects and it's nice to have this stuff written down as well.

I've wrote a few blog posts before on [dev.to](https://dev.to/jacqueslucke/a-c-micro-optimization-exercise-3p65) and [code.blender.org](https://code.blender.org/author/jacqueslucke/). However, I'd also like to write about topics unrelated to Blender and programming. I'm not super interested in using existing platforms (like Medium etc.) to share these posts that I don't have a good amount of control over. Therefore, the my logical choice is to just have my own blog.
...
- I want the website to be small. It should not come with a bunch of bloat that increases the download size but does not increase its value.
- I want to know some usage stats but I don't want to have to show a cookie banner.
- No ads. There is no intention to make money from people just reading the site.
...
What I do now is to just have some css variables for the color theme at the root level which are changed when there is a `.dark` class on the root element. See the code [here](https://github.com/JacquesLucke/jlucke.com/blob/7e1f1f38e8750632cae670935d95394d3df46ac2/src/styles/global.css#L5-L38). The way I integrated that with tailwind feels a bit weird but it seems to work well. I may still have to iterate on this again in the future.

### Images

Besides avoiding large client-side Javascript libraries, the biggest thing affect page size is images. By default, images (especially photos) are often way bigger than necessary to display them on the web. Thankfully, Astro has [makes it easy](https://docs.astro.build/en/guides/images/) to resize and compress images as part of the build process.
...
The gathered analytics data is available publicly [here](https://plausible.jlucke.com/jlucke.com).
...
- `10.44$ / year` for the `jlucke.com` domain.
- `3,92€ / month` for my server running Plausible.
- `0,60€ / month` for the ipv4 for the server running Plausible.
...
Overall, I'm quite happy with the outcome. No I just have to fill the site with more content, starting with this first blog post.

The source code is available [here](https://github.com/JacquesLucke/jlucke.com).
